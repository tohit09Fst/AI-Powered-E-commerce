 ...
 * Uses Cohere if API key is available, otherwise falls back to AI Gateway
 */
export function createShoppingAgent({ userId }: ShoppingAgentOptions) {
  ...
  console.log("[ShoppingAgent] Checking for Cohere API Key:", cohereApiKey ? `Found (${cohereApiKey.substring(0, 6)}...)` : "Not found");

  if (cohereApiKey) {
    // Use Cohere if API key is configured
    console.log("[ShoppingAgent] Using Cohere model: command-r-08-2024");
    ...
    // Fall back to AI Gateway (will fail if no credit card, but that's expected)
    console.log("[ShoppingAgent] Using AI Gateway (requires credit card)");
    return new ToolLoopAgent({
