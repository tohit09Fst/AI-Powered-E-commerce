  ...
  execute: async ({ query, category, material, color, minPrice, maxPrice }) => {
    ...
    try {
      ...
      const formattedProducts: SearchProduct[] = (
        ...
        // Use product ID as fallback if slug is missing
        const slug = product.slug || product._id;

        // Log warning for products without proper slugs
        if (!product.slug) {
          console.warn(
            `[SearchProducts] Product "${product.name}" (${product._id}) is missing slug, using ID as fallback`
          );
        }
